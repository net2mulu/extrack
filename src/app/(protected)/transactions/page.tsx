        ...
        <div key={date}>
          ...
          <div className="bg-card rounded-xl border border-border/50 overflow-hidden shadow-sm">
            {txs.map((t, i) => (
              <div
                key={t.id}
                className={`flex justify-between items-center p-3 sm:p-4 hover:bg-muted/50 transition-colors ${
                  i !== txs.length - 1 ? "border-b border-border/50" : ""
                }`}
              >
                <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                  <div className="bg-secondary/50 w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center shrink-0">
                    <CategoryIcon iconName={t.category?.icon} className="h-4 w-4 sm:h-5 sm:w-5" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="font-semibold text-xs sm:text-sm truncate">
                      {t.note || t.category?.name || "Expense"}
                    </div>
                    <div className="text-xs text-muted-foreground truncate">{t.category?.name}</div>
                  </div>
                </div>
                <div
                  className={`font-mono font-bold text-xs sm:text-sm shrink-0 ml-2 ${
